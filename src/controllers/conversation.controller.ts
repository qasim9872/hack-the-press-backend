...
import { CustomLogger } from "@root/utils/logger/helper"

export async function getResponse(logger: CustomLogger, to: string, intent: string, input?: string) {
  const bot = await MyBotModel.findByPhoneNumber(to)

  if (!bot) {
    const message = `No bot found for phone number: ${to}`
    logger.error(message)
    logger.slack.error(message)
  ...
  if (["NO_MATCH", "NO_INPUT"].includes(intent)) {
    const message = `WARNING: intent classified as: ${intent}. ${input}`
    logger.warn(message)
    logger.slack.warn(message)
  }
...
export async function init(logger: CustomLogger, to: string) {
  const greetings = "General_Greetings"

  return getResponse(logger, to, greetings)
}

export async function process(logger: CustomLogger, to: string, input: string) {
